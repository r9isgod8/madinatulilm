 ...
 */
if ( !function_exists( 'mirasat_get_template_part' ) ) {

	function mirasat_get_template_part( $slug, $name = null, array $mirasat_params = array() ) {
 ...
 * Print H1 header
*/
if ( !function_exists( 'mirasat_the_h1' ) ) {

	function mirasat_the_h1() {

		$title = mirasat_get_the_h1();
		if ( !empty($title) ) echo '<h1 class="header ltx-check-scroll ltx-glitch" data-text="' . esc_attr( $title ) . '">' . esc_html( $title ) . '</h1>';
	}
}

if ( !function_exists( 'mirasat_get_the_h1' ) ) {

	function mirasat_get_the_h1() {

		global $wp_post;
		
		if ( is_home() ) {

			$title = esc_html__( 'All Blog Posts', 'mirasat' );
		} 
			else
		if ( is_front_page() ) {

			$title = esc_html__( 'Home', 'mirasat' );
		}
			else
		if ( is_year() ) {

			$title = sprintf( esc_html__( 'Year Archives: %s', 'mirasat' ), get_the_date( 'Y' ) );
		}
			else				
		if ( is_month() ) {

			$title = sprintf( esc_html__( 'Month Archives: %s', 'mirasat' ), get_the_date( 'F Y' ) );
		}
			else
		if ( is_day() ) {

			$title = sprintf( esc_html__( 'Day Archives: %s', 'mirasat' ), get_the_date() );
		}
			else
		if ( is_category() ) {
		...
		if ( is_tag() ) {
			...
			$title = sprintf( esc_html__( 'Tag: %s', 'mirasat' ), single_tag_title( '', false ) );
		}
			else
		if ( is_tax() ) {
		...
		if ( is_search() ) {
			...
			$title = sprintf( esc_html__( 'Search Results: %s', 'mirasat' ), get_search_query() );
		} 
			else				
		if ( is_author() ) {
			...
			$title = sprintf( esc_html__( 'Author: %s', 'mirasat' ), $q->display_name );
		} 
			else
		...
		if ( is_attachment() ) {
			...
			$title = sprintf( esc_html__( 'Attachment: %s', 'mirasat' ), get_the_title() );
		}
			else
		if ( is_404() ) {

			$title = esc_html__( '404 Not Found', 'mirasat' );
		}
			else {
 ...
 */
if ( !function_exists( 'mirasat_add_current_nav_class' ) ) {

	function mirasat_add_current_nav_class( $classes, $item ) {
...
add_action( 'nav_menu_css_class', 'mirasat_add_current_nav_class', 10, 2 );


/**
 * Manual excerpt generation
 */
if ( !function_exists( 'mirasat_excerpt_set' ) ) {

	function mirasat_excerpt_set() {
	...
	add_filter( 'excerpt_length', 'mirasat_excerpt_set', 999 );
}


if ( !function_exists( 'mirasat_excerpt' ) ) {
	
	function mirasat_excerpt( $content, $excerpt = 0 ) {
		...
		if ( ! empty( $post->post_content ) &&
			...
			$content = mirasat_cut_excerpt( $post->post_content , $excerpt );
		}
...
if ( !function_exists( 'mirasat_cut_excerpt' ) ) {
	
	function mirasat_cut_excerpt( $content = '', $excerpt = 0 ) {

		$cut = false;
		$excerpt_more = apply_filters( 'excerpt_more', ' ...' );
		$content = mirasat_get_content( $content );
		...
		$content = mirasat_strip_tags( $texts, $cut );
 ...
 */
if ( !function_exists( 'mirasat_cut_text' ) ) {

	function mirasat_cut_text( $text, $cut = 300, $aft = ' ...' ) {
 ...
 */
if ( !function_exists( 'mirasat_get_content' ) ) {
	
	function mirasat_get_content( $content = '' ) {
...
if ( !function_exists( 'mirasat_strip_tags' ) ) {
	
	function mirasat_strip_tags( $texts = array(), $cut = 0 ) {
 ...
 * @return bool
 */
if ( !function_exists( 'mirasat_is_wc' ) ) {

	function mirasat_is_wc($tag, $attr='') {
 ...
 */
if ( !function_exists('mirasat_is_vc') ) {

    function mirasat_is_vc() {
 ...
 */
if ( !function_exists( 'mirasat_plugin_is_active' ) ) {
	
	function mirasat_plugin_is_active( $plugin_var, $plugin_dir = null ) {
 ...
 */
if ( !function_exists( 'mirasat_admin_css' ) ) {
	
	function mirasat_admin_css() {

		wp_enqueue_style( 'mirasat-admin', get_template_directory_uri() . '/assets/css/admin.css', false, '1.0.0' );
	}
}
add_action( 'admin_enqueue_scripts', 'mirasat_admin_css' );

/**
 * Return inverted contrast value of color
 */
if ( !function_exists( 'mirasat_rgb_contrast' ) ) {
	
	function mirasat_rgb_contrast($r, $g, $b) {
 ...
 */
if ( !function_exists( 'mirasat_color_change' ) ) {
	
	function mirasat_color_change( $hex, $percent ) {
...
function mirasat_adjustBrightness($hex, $steps) {
 ...
 * @return array();
 */
if ( !function_exists( 'mirasat_get_footer_cols_num' ) ) {

	function mirasat_get_footer_cols_num() {
 ...
 */
if ( !function_exists( 'mirasat_get_navbar_layout' ) ) {

	function mirasat_get_navbar_layout( $default = null ) {
		...
		if ( ( !empty( $navbar_layout) AND empty( $mirasat_theme_config['navbar'][$navbar_layout] ) )
 ...
 * Return navbar menu
*/
if ( !function_exists( 'mirasat_get_wp_nav_menu' ) ) {

	function mirasat_get_wp_nav_menu() {
 ...
 */
if ( !function_exists( 'mirasat_get_sections' ) ) {

	function mirasat_get_sections() {
 ...
 */
if ( !function_exists( 'mirasat_get_pageheader_layout' ) ) {

	function mirasat_get_pageheader_layout() {
 ...
 */
if ( !function_exists( 'mirasat_get_pageheader_class' ) ) {

	function mirasat_get_pageheader_class() {
		
		$mirasat_header_class = array();
		$mirasat_h1 = mirasat_get_the_h1();

		if ( !empty($mirasat_h1) ) {

			$mirasat_header_class[] = ' header-h1 ';
		}
		...
		if ( function_exists('FW') ) {
			...
			if ( $header_fixed == 'fixed' ) {
				...
				$mirasat_header_class[] = ' header-parallax ';
			}
		}

		if ( function_exists( 'bcn_display' ) && !is_front_page() ) {

			$mirasat_header_class[] = ' hasBreadcrumbs';
		}
		...
		$mirasat_header_class[] = ' wrapper-navbar-layout-' . $navbar_layout;



		return implode( ' ', $mirasat_header_class );
	}
}
 ...
 */
if ( !function_exists( 'mirasat_get_pageheader_wrapper' ) ) {

	function mirasat_get_pageheader_wrapper() {
 ...
 */
if ( !function_exists( 'mirasat_check_active_sidebar' ) ) {

	function mirasat_check_active_sidebar() {
 ...
 */
if ( !function_exists( 'mirasat_get_wc_sidebar_pos' ) ) {

	function mirasat_get_wc_sidebar_pos() {

		if ( mirasat_is_wc('product') ) {

			$mirasat_sidebar = false;
		}
			else {

			$mirasat_sidebar = 'left';
		}
		...
		if ( function_exists( 'FW' ) ) {
			...
			if ( mirasat_is_wc('product') ) {
				...
				$mirasat_sidebar = fw_get_db_settings_option( 'shop_post_sidebar' );
			}	
				else {

				$mirasat_sidebar = fw_get_db_settings_option( 'shop_list_sidebar' );
			}
 ...
 */
if ( !function_exists( 'mirasat_custom_css' ) ) {

	function mirasat_custom_css( $css = null ) {
 ...
 */
if ( !function_exists( 'mirasat_find_http' ) ) {

	function mirasat_find_http( $string ) {
 ...
 * 
 */
function mirasat_the_img_srcset( $attachment_id, $sizes_hooks, $sizes_media ) {
...
$mirasat_current_scheme =  apply_filters ('mirasat_current_scheme', array());
